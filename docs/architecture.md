...
- Context is stored outside source files in a self-ignored `.thread-keep/` directory at the worktree root. SQLite/FTS5 is the local current-state projection; content-addressed files store immutable committed context objects. If only the legacy `.git/thread-keep/` database exists, the first storage command copies it through SQLite's online backup API, validates the staged database, copies immutable objects, and atomically publishes `.thread-keep/` without modifying the legacy source.
...
- The local store and working set are scoped to the Git worktree, branch-derived context ref and source SHA. Linked worktrees have independent databases, immutable object directories, and pending notes; remote context synchronization is the sharing boundary.
