...
- Pack resolution prefers signed managed activation, then a legacy fixed local executable, then a target-qualified PyPI-bundled fallback supplied by the Python launcher. `indexers install --detected` installs only detected missing packs; `indexers sync --detected [--version X.Y.Z]` is the explicit upgrade/pin/rollback boundary and therefore overrides the bundled fallback. A release binary validates a signed official manifest, platform artifact, byte size and SHA-256, publishes an immutable content-addressed executable, and atomically activates versioned metadata. `init` and `update` never install or replace packs.
