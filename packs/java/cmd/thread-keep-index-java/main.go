...
func run(ctx context.Context, arguments []string, input io.Reader, output io.Writer) error {
	...
	return json.NewEncoder(output).Encode(response{ProtocolVersion: protocolVersion, Indexer: identity{ID: "thread-keep-index-java", Version: indexerVersion}, Language: "java", Entities: entities, Diagnostics: diagnostics})
}
