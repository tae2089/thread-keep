...
func run(ctx context.Context, arguments []string, input io.Reader, output io.Writer) error {
	if len(arguments) != 2 || arguments[0] != "index" || arguments[1] != "--protocol-version=1" {
		return errors.New("usage: thread-keep-index-javascript index --protocol-version=1")
	}
	...
	if inputRequest.ProtocolVersion != protocolVersion || inputRequest.Language != "javascript" || !filepath.IsAbs(inputRequest.RepositoryRoot) || inputRequest.SourceSHA == "" {
	...
	return json.NewEncoder(output).Encode(response{ProtocolVersion: protocolVersion, Indexer: identity{ID: "thread-keep-index-javascript", Version: indexerVersion}, Language: "javascript", Entities: entities, Diagnostics: diagnostics})
}
...
func index(ctx context.Context, input request) ([]entity, []string, error) {
	...
	if err := parser.SetLanguage(tree_sitter.NewLanguage(tree_sitter_javascript.Language())); err != nil {
		return nil, nil, fmt.Errorf("set JavaScript parser language: %w", err)
	}
	...
	for _, relative := range input.Files {
		...
		entities = append(entities, collect(root, contents, filepath.ToSlash(relative), "")...)
		tree.Close()
	}
...
func collect(node *tree_sitter.Node, source []byte, path, owner string) []entity {
	var entities []entity
	nextOwner := owner
	if kind, ok := declarationKind(node.Kind()); ok {
		...
		if name != "" {
			...
			if ownsDeclarations(kind) {
				nextOwner = qualifiedName
			}
		}
	}
	if node.Kind() == "variable_declarator" {
		...
		if name != "" && value != nil {
			...
			case "arrow_function", "function_expression", "generator_function":
				entities = append(entities, newEntity(node, source, path, "function", name, qualifiedName))
			case "class":
				entities = append(entities, newEntity(value, source, path, "class", name, qualifiedName))
				nextOwner = qualifiedName
			}
		}
	}
	if node.Kind() == "field_definition" && owner != "" {
		name := fieldText(node, "property", source)
		value := node.ChildByFieldName("value")
		if name != "" && value != nil && (value.Kind() == "arrow_function" || value.Kind() == "function_expression" || value.Kind() == "generator_function") {
			entities = append(entities, newEntity(node, source, path, "method", name, qualify(owner, name)))
		}
	}
...
func ownsDeclarations(kind string) bool {
