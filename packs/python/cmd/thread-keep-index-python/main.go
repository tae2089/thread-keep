...
func run(ctx context.Context, arguments []string, input io.Reader, output io.Writer) error {
	if len(arguments) != 2 || arguments[0] != "index" || arguments[1] != "--protocol-version=1" {
		return errors.New("usage: thread-keep-index-python index --protocol-version=1")
	}
	...
	if inputRequest.ProtocolVersion != protocolVersion || inputRequest.Language != "python" || !filepath.IsAbs(inputRequest.RepositoryRoot) || inputRequest.SourceSHA == "" {
...
func collect(node *tree_sitter.Node, source []byte, path string, owner scope) []entity {
	...
	case "function_definition":
		...
		if name != "" {
			...
			nextOwner = scope{qualifiedName: qualifiedName, kind: "function"}
		}
	}
