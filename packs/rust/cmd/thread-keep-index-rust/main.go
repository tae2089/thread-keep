...
func run(ctx context.Context, arguments []string, input io.Reader, output io.Writer) error {
	...
	return json.NewEncoder(output).Encode(response{ProtocolVersion: protocolVersion, Indexer: identity{ID: "thread-keep-index-rust", Version: "1"}, Language: "rust", Entities: entities, Diagnostics: diagnostics})
}
