...
func run(ctx context.Context, arguments []string, input io.Reader, output io.Writer) error {
	if len(arguments) != 2 || arguments[0] != "index" || arguments[1] != "--protocol-version=1" {
		return errors.New("usage: thread-keep-index-rust index --protocol-version=1")
	}
	...
	if inputRequest.ProtocolVersion != protocolVersion || inputRequest.Language != "rust" || !filepath.IsAbs(inputRequest.RepositoryRoot) || inputRequest.SourceSHA == "" {
	...
	return json.NewEncoder(output).Encode(response{ProtocolVersion: protocolVersion, Indexer: identity{ID: "thread-keep-index-rust", Version: indexerVersion}, Language: "rust", Entities: entities, Diagnostics: diagnostics})
}
...
func index(ctx context.Context, input request) ([]entity, []string, error) {
	...
	if err := parser.SetLanguage(tree_sitter.NewLanguage(tree_sitter_rust.Language())); err != nil {
		return nil, nil, fmt.Errorf("set Rust parser language: %w", err)
	}
...
func collect(node *tree_sitter.Node, source []byte, path string, owner scope) []entity {
	...
	case "mod_item":
		...
		if name != "" {
			nextOwner = scope{qualifiedName: qualify(owner.qualifiedName, name), kind: "module"}
		}
	case "struct_item", "union_item", "type_item":
		name := fieldText(node, "name", source)
		if name != "" {
			...
			nextOwner = scope{qualifiedName: qualifiedName, kind: "type"}
		}
	case "enum_item":
		name := fieldText(node, "name", source)
		if name != "" {
			qualifiedName := qualify(owner.qualifiedName, name)
			entities = append(entities, newEntity(node, source, path, "enum", name, qualifiedName))
			nextOwner = scope{qualifiedName: qualifiedName, kind: "enum"}
		}
	case "trait_item":
		name := fieldText(node, "name", source)
		if name != "" {
			qualifiedName := qualify(owner.qualifiedName, name)
			entities = append(entities, newEntity(node, source, path, "interface", name, qualifiedName))
			nextOwner = scope{qualifiedName: qualifiedName, kind: "trait"}
		}
	case "impl_item":
		target := implTargetName(fieldText(node, "type", source))
		if target != "" {
			nextOwner = scope{qualifiedName: qualify(owner.qualifiedName, target), kind: "impl"}
		}
	case "function_item", "function_signature_item":
		name := fieldText(node, "name", source)
		if name != "" {
			kind := "function"
			if isMemberScope(owner.kind) {
				kind = "method"
			}
			qualifiedName := qualify(owner.qualifiedName, name)
	...
	case "associated_type":
		name := fieldText(node, "name", source)
		if name != "" && owner.qualifiedName != "" && isMemberScope(owner.kind) {
			qualifiedName := qualify(owner.qualifiedName, name)
			entities = append(entities, newEntity(node, source, path, "type", name, qualifiedName))
		}
	}
	for index := uint(0); index < node.NamedChildCount(); index++ {
		entities = append(entities, collect(node.NamedChild(index), source, path, nextOwner)...)
	}
	return entities
}

func isMemberScope(value string) bool {
	return value == "trait" || value == "impl"
}

func implTargetName(value string) string {
