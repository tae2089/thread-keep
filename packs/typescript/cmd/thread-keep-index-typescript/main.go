...
func run(ctx context.Context, arguments []string, input io.Reader, output io.Writer) error {
	if len(arguments) != 2 || arguments[0] != "index" || arguments[1] != "--protocol-version=1" {
		return errors.New("usage: thread-keep-index-typescript index --protocol-version=1")
	}
	...
	if inputRequest.ProtocolVersion != protocolVersion || inputRequest.Language != "typescript" || !filepath.IsAbs(inputRequest.RepositoryRoot) || inputRequest.SourceSHA == "" {
	...
	return json.NewEncoder(output).Encode(response{ProtocolVersion: protocolVersion, Indexer: identity{ID: "thread-keep-index-typescript", Version: indexerVersion}, Language: "typescript", Entities: entities, Diagnostics: diagnostics})
}
...
func index(ctx context.Context, input request) ([]entity, []string, error) {
	...
	for _, relative := range input.Files {
		...
		if err := parser.SetLanguage(language); err != nil {
			return nil, nil, fmt.Errorf("set parser language for %s: %w", relative, err)
		}
...
func collect(node *tree_sitter.Node, source []byte, path, owner string) []entity {
	var entities []entity
	nextOwner := owner
	if node.Kind() == "internal_module" || node.Kind() == "module" {
		if name := fieldText(node, "name", source); name != "" {
			nextOwner = qualify(owner, name)
		}
	}
	...
	if node.Kind() == "variable_declarator" || node.Kind() == "public_field_definition" {
		value := node.ChildByFieldName("value")
		if value != nil && (value.Kind() == "arrow_function" || value.Kind() == "function") {
			if name := fieldText(node, "name", source); name != "" {
				kind := "function"
				qualifiedName := qualify(owner, name)
				if owner != "" && node.Kind() == "public_field_definition" {
					kind = "method"
				}
				entities = append(entities, newEntity(node, source, path, kind, name, qualifiedName))
			}
